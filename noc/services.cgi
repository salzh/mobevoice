...
sub do_view_history(){
	...
	#---------------------------------------------------
	$sql = qq[
		SELECT action_log.id,unix_timestamp(date)
		from action_log,action_log_type 
		where action_log.type=action_log_type.id
		and service_id='$service_id'
		and action_log_type.group like '$type'
	];
	...
	foreach (keys %hash){
		$collected_ids{substr("000000000000000000".$hash{$_},-15,15).substr("000000000000000000".$_,-15,15)."1"}++;
	}
...
#&template_print("template.html",%t);
	...
	if ($logs_ids ne "") {
		...
		action_log_type.group,action_log_type.title,action_log_type.description,action_log_type.id
		from action_log,action_log_type 
		where action_log.type=action_log_type.id and action_log.id in ($logs_ids)
		and action_log_type.group like '$type'
		limit 0,1000
		";
		%logs_data = database_select_as_hash($sql,"noc_id,value_old,value_new,group,title,description,logtype");
	}
...
#%calls_data = database_select_as_hash($sql,"ani,did,dst,billing_id,error_id,info_id");
				...
				%carrier = &get_carrier_by_number($calls_data{$id}{dst});
				$html_history .= "Forward to ".&format_E164_number($calls_data{$id}{dst},"USA"). " : " . $carrier{carrier_name} .  "</td>";
...
#%logs_data = database_select_as_hash($sql,"noc_id,value_old,value_new,group,title,description");
			if (exists($logs_data{$id})) {
				...
				if ($logs_data{$id}{description} eq "") {
					...
					if($logs_data{$id}{logtype} eq "status:new"){
						%carrier = &get_carrier_by_number($logs_data{$id}{value_old});
						$tmp .= " : " . $carrier{carrier_name} ;
					}
	...
	@types = ("All", "Commissions", "Unknown", "Service action", "Recharge", "Security", "NOC action", "Payment profile", "Recharge (details)", "Sign-in", "Coupons", "Payment profile (details)");
	#adm_error(Dumper(\%types));
	$typeSelect = "<select id='typeFilter'>";
	for(sort @types){
		$typeSelect .= "<option>$_</option>";
	}
	$typeSelect .= "</select>";
	$typeSelect .= qq[
	<script>
	\$('#typeFilter').attr('value', '$type');
	\$("#typeFilter").live("change", function(e){
		var type = \$("#typeFilter")[0].value;
		top.location = "services.cgi?service_id=$service_id&action=view_history" + "&type=" + type;
	});
	</script>
	];
    #
    ...
    $t{dic}{content}	= qq[
			...
			<table border=0 colspan=0 cellpadding=0 cellspacing=0 class=WindowsTable width=100% style=margin-top:13px; xonclick="sortColumn(event)">
				<thead>
					...
					<tr>
					<td width=80>Date</td>
					<td width=100>Type $typeSelect </td>
					<td >Action</td>
...
sub do_detail_call(){
	...
	# print page
	$sql = "
	SELECT 1,1,unix_timestamp(datetime_start),id,ani,did,dst,billing_id,rate_slot,error_id,info_id,ast_unique_id,system_host 
	FROM calls_log where id='$call_log_id' 
	";
	%hash = database_select_as_hash($sql, "flag,date,id,ani,did,dst,billing_id,rate_slot,error_id,info_id,ast_unique_id,system_host");
	if ($hash{1}{flag} ne 1) {
		...
		$t{dic}{content} .= "Rate Slot: ".	 $call{rate_slot} ."<br>";
...
#		@lines = split(/\n/,$raw->decoded_content);
#	}
	$sql = "SELECT   1,1, A.text from log_debug A,log_debug_reference B where A.id = B.log_debug_id and B.reference_name='call_log_id' and B.reference_value='$call_log_id'"; 
	%hash = database_select_as_hash($sql, "flag,text");
	if ($hash{1}{flag} eq 1) {
		$hash{1}{text} =~ /hangup_cause\=(\d+)/;
		$hangup_cause = $1;
		$t{dic}{content} .= "<br>Hangup Cause: ".	 $hangup_cause ."<br>";
		$hash{1}{text} =~ /Rate data asterisk_string = ([^\n]+)/;
		$ast_string = $1;
		$t{dic}{content} .= "Asterisk String: ".	 $ast_string ."<br>";
		 @lines = split(/\n/,$hash{1}{text});
	}

			$buf = "\n<table id=debugtable class='dataTable' style='width:500px'>"
			. " <thead><tr><th>Seq</th><th>Info</th></tr></thead><tbody>\n";
			$ln = 1;
			foreach (@lines){
				#$buf .=$_."<br>\n";
				...
				if ( (index($tmp2,"!") ne -1) && (index($tmp2,"debug") ne -1) ) {
					...
					foreach $tt0 (split(/\!/,$tmptmp2)) {
						...
						$tt1=">"; $tt2="&gt;"; $tt0 =~ s/$tt1/$tt2/eg; 
						#$buf .= "<tr><tr>$ln</td><td>$tmptmp1 $c</td><td>$tt0</td></tr>\n";
						$c++;
						$ln++;
					}
				} else {
					...
					$tt1=">"; $tt2="&gt;"; $tmp2 =~ s/$tt1/$tt2/eg; 
					$buf .= "<tr><td>$ln</td><td> $tmp$tmp2</td></tr>\n";
					$ln++;
				}
			}
			$buf .= "</tbody></table>";
...
sub do_detail_commission(){
	...
	$sql = "
	SELECT 1,1,
	creation_date,activation_date_1,activation_date_2, status,
	credit_id,invoice_id,from_service_id,deep,
	engine,value,age_discount_value_before,age_discount_percentage,
	percentage, apply_mode, value_base, age_id
	FROM service_commission 
	where id='$commission_id' 
	";
	%hash = database_select_as_hash($sql,"flag,creation_date,activation_date_1,activation_date_2,status,credit_id,invoice_id,from_service_id,deep,engine,value,age_discount_value_before,age_discount_percentage,percentage,apply_mode,value_base,age_id");
    ...
    $t{dic}{my_url}	= $my_url;
    $t{dic}{content}	= qq[
	<link rel="stylesheet" type="text/css" href="//ajax.aspnetcdn.com/ajax/jquery.dataTables/1.8.2/css/jquery.dataTables.css">
	<link rel="stylesheet type="text/css" href="/noc/design/datatable_table.css" >
	<table id='debug-comission'>
	<thead>
	<tr><th>name</th><th>Value</th></tr>
	</thead>
	<tbody>
	<tr>
    <td>commission id</td><td>$commission_id</td>
	</tr>
	];
	for $n(sort keys %commission_data){
		$t{dic}{content}	.= qq[
			<tr><td>$n</td><td> $commission_data{$n}</td></tr>
		];
	}
...
<script>	
    ...
    \$('#debug-comission').dataTable({"bPaginate": false});
} );
</script>
...
sub do_search(){
	...
	foreach $index (1..4) {
		...
		if( index($filter,"route_") eq 0){
			$route = substr($filter, 6, 10);
			$sql = qq[
select cl.service_id, 1 from calls_log cl
where cl.rate_slot=$route
and cl.datetime_start > DATE_SUB(now(), interval 7 day)
and (cl.datetime_start in (select max(datetime_start) from calls_log where service_id=cl.service_id))
order by cl.id DESC
			];
		}
    ...
    if ($form{export} eq "1"){
	    ...
	    print "\n";
	    print "Email Address,First Name,Last Name,Work Phone,Home Phone,Country,Notes,Custom Field 1,Custom Field 2,Custom Field 3,Last Call,Balance,Status,Referer\n";
		...
		if ($selected_services_ids_raw ne "") {
			# query os dados
			$sql = "
				select service.id,service.name,service.email,service_invite.id,service.balance,service_status.name 
					from service,service_invite,service_status 
					where service.id in ($selected_services_ids_raw) and service.id=service_invite.service_id
					and service_status.id =service.status 
					order by service.last_change 
			";
			%hash = database_select_as_hash($sql,"name,email,invite,balance,status_name");
			...
			%hash2 =  database_select_as_hash(
			"SELECT A.service_id,B.name FROM service_tree A,service B where A.parent_service_id =B.id and A.service_id in ($selected_services_ids_raw) ");
			
			foreach (keys %hash) {
				
				$tmp1 = ($hash1{$_} ne "") ? &format_time_gap($hash1{$_}) : "&nbsp;";
				$tmp2 = &format_number($hash{$_}{balance},2);
				if (1){ 							#$hash{$_}{email} ne "") { some email fields were blank and these were not written in csv
				    print "\"$hash{$_}{email}\",";
				    print "\"$hash{$_}{name}\",";
				    print "\"\",";
				    print "\"\",";
	...
	if ($form{sms} eq "1"){
		...
		if ($selected_services_ids_raw ne "") {
			# query os dados
			#unique ani is selected
			$sql = "select service_id,ani from service_signin where service_id in ($selected_services_ids_raw)";
			
			#for multiple ani
			#$sql = "select concat(service_id, ani),ani from service_ani where service_id in ($selected_services_ids_raw)";
		...
		$form{ani_to} = "";
		$qtd=0;
		foreach (sort keys %selected_ani) {$form{ani_to} .= "+$_,";$qtd++;}
		%t = ();
		$t{dic}{my_url}		= $my_url;
		$t{dic}{title}		= "Send SMS";
		$t{dic}{content}	= qq[
		Please wait...
		<form action='https://www.zenofon.com/noc/sms.cgi' method=post>
			<div>
				<textarea name=to style="width:300px; height:200px;">$form{ani_to}</textarea><br>
				<textarea name=body style="width:300px; height:100px;">Message...</textarea>
				<br>
				$qtd ANIs<br>
				<a href=javascript:history.back()>Go back</a>
				<br>
				<input type=submit name='submit' value='send' style="height:100px;width:200px;">
			</div>
		</form>
		];
		&template_print("template.html",%t);
	...
	if ($selected_services_ids_raw ne "") {
		...
		%hash3 = database_select_as_hash("SELECT service_id,group_concat(ani) FROM service_ani where   service_id in ($selected_services_ids_raw) group by service_id ");
		...
		foreach $id (@selected_services_ids) {
			...
			$services_data{$id}{ani} = $hash3{$id};
			$tmp =$hash3{$id};
		 	if (length($tmp) >30) {
				 $tmp =substr($tmp,0,30).'...';
			}
			$services_data{$id}{ani} = $tmp ;
    ...
    foreach $id (@selected_services_ids) {
		...
		<td valign=top>$services_data{$id}{ani}&nbsp;</td>		
	...
	foreach $index (1..4) {
		...
		<option value=""></option>
		<option value="">By Last Call Route</option>
		<option $select_filter{route_1}  	value=route_1	>&nbsp;&nbsp;-&nbsp;Last Call Through route 1 </option>
		<option $select_filter{route_2}  	value=route_2	>&nbsp;&nbsp;-&nbsp;Last Call Through route 2 </option>
		<option $select_filter{route_3}  	value=route_3	>&nbsp;&nbsp;-&nbsp;Last Call Through route 3 </option>
		<option $select_filter{route_4}  	value=route_4	>&nbsp;&nbsp;-&nbsp;Last Call Through route 4 </option>
		<option $select_filter{route_5}  	value=route_5	>&nbsp;&nbsp;-&nbsp;Last Call Through route 5 </option>
		<option $select_filter{route_6}  	value=route_6	>&nbsp;&nbsp;-&nbsp;Last Call Through route 6 </option>
		<option $select_filter{route_7}  	value=route_7	>&nbsp;&nbsp;-&nbsp;Last Call Through route 7 </option>
		<option $select_filter{route_8}  	value=route_8	>&nbsp;&nbsp;-&nbsp;Last Call Through route 8 </option>
		<option $select_filter{route_9}  	value=route_9	>&nbsp;&nbsp;-&nbsp;Last Call Through route 9 </option>
		];
	...
	<br>
		...
		<br><br>
		For all $quantity_string services:
		<button name=email class=button type=submit value=1>Send email</button>
		<button name=export class=button type=submit value=1>Export to constantcontact.com</button>
		<button name=sms type=submit value=1>Send SMS</button>
		<br>
	</form>
...
$dddd .= "DST query - $id - $list{$id}{service_id} - $list{$id}{dst} - $list{$id}{flag}<br>";
	...
	if ($form{sms} eq "1"){
		...
		if ($selected_services_ids_raw ne "") {
			...
			from service_signin
			where 
			service_id in ($selected_services_ids_raw)
			";
			%hash = database_select_as_hash($sql);
			foreach (keys %hash) {
				if ($hash{$_} eq "") {next}
		...
		$form{ani_to} = "";
		$qtd=0;
		foreach (sort keys %selected_ani) {$form{ani_to} .= "$_\n";$qtd++;}
		%t = ();
		$t{dic}{my_url}		= $my_url;
		$t{dic}{title}		= "Send SMS";
		$t{dic}{content}	= qq[
		...
		<form action=dummy.cgi method=post>
			<div>
				<textarea name=email_to style="width:300px; height:300px;">$form{ani_to}</textarea>
				<br>
				$qtd emails<br>
				<a href=javascript:history.back()>Go back</a>
				<br>

...
# main loop
#=======================================================
$my_url = "radio.reports.hourly.cgi";
$action = $form{action};
$action="hourly";
if	($action eq "hourly")	{ &do_radio_report_overview();					}
else											{ 	}
exit;
#=======================================================
...
sub do_radio_report_overview(){
    ...
    # prepare year/month select and values
	#==========================================
	%yearmonth_list = &database_select_as_hash("SELECT distinct DATE_FORMAT(hour,'\%Y-\%m-\%d'),1 FROM radio_log_listen_session_hourly order by DATE_FORMAT(hour,'\%Y-\%m-\%d') desc  limit 7 ");
	...
	#==========================================
	$plot_idx = 0;
	 foreach $id (sort{$b cmp $a}  keys %yearmonth_list) {
	warning('id:'.$id);
	$filter_yearmonth_month_day_01 = $id;
	$sql = "
		select
			DATE_FORMAT(hour,'\%H'),
			minutes, new_clients,return_clients,total_clients
		from
			radio_log_listen_session_hourly
		where
			hour >= '$filter_yearmonth_month_day_01' and hour < DATE_ADD('$filter_yearmonth_month_day_01', INTERVAL 1 DAY)
			and $filter_daily_station_sql
	...
	%plot = ();
	$plot{uid} = "minutes_hour_by_hour_".$id;
	$plot{x} = 400;
	$plot{y} = 200;
	$plot{series}{1}{name} 	= "Acculated minutes by hour / ".$id;
	...
	foreach $d (@days) {
		$dd = substr("00".$d,-2,2);
		$plot{data}{$slice_index}{1} = $sum_minutes =  $sum_minutes + int($data{$dd}{minutes});
	...
	$minutes_hour_by_hour{$plot_idx} = $plot{html};
	$plot_idx++;
	}
	#
	#
    ...
    $t{my_url}	= $my_url;
    $t{title}	= "Daily Phone radio overview";
	$t{content} = qq[
	...
	We just collect latest 7 days 
	<br>
	<br>
	<table border=0 colspan=0 cellpadding=0 cellspacing=0 class=clear>
		<td valign=top>$minutes_hour_by_hour{0}</td>
		
	</table>

  <table border=0 colspan=0 cellpadding=0 cellspacing=0 class=clear>
                <td valign=top>$minutes_hour_by_hour{1}</td>
                
        </table>
  <table border=0 colspan=0 cellpadding=0 cellspacing=0 class=clear>
                <td valign=top>$minutes_hour_by_hour{2}</td>
                
        </table>
  <table border=0 colspan=0 cellpadding=0 cellspacing=0 class=clear>
                <td valign=top>$minutes_hour_by_hour{3}</td>
                
        </table>
  <table border=0 colspan=0 cellpadding=0 cellspacing=0 class=clear>
                <td valign=top>$minutes_hour_by_hour{4}</td>
                
        </table>
  <table border=0 colspan=0 cellpadding=0 cellspacing=0 class=clear>
                <td valign=top>$minutes_hour_by_hour{5}</td>

        </table>
  <table border=0 colspan=0 cellpadding=0 cellspacing=0 class=clear>
                <td valign=top>$minutes_hour_by_hour{6}</td>

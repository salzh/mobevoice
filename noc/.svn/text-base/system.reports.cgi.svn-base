...
# system reports
#=======================================================
# system wide data reports. Remember that here, user can see
# ALL stations, channels, logs, calls etc etc.
# NEVER leave other users (radio owners for example) access this actions
#=======================================================
require "include.cgi";
...
if (&active_user_permission_get("radio.data.access") eq 2) {
	...
	$allow_stationid_str = ",$stations_allow,";
}
...
sub do_radio_report_now(){
    ...
    foreach (sort{$stations{$b}{calls} <=> $stations{$a}{calls}} keys %stations){
    	if ($stations{$_}{calls} > 0) {
		    $content .= "<tr class='filter_$stations{$_}{title}'><td>$stations{$_}{title}</td><td class=ar>$stations{$_}{calls}</td></tr>";
    ...
    $sql = "
		...
		FROM radio_data_station_stream as s, radio_data_station as r
    ...
    foreach $stream_id (sort{$hash{$b}{percentage_1} <=> $hash{$a}{percentage_1}} keys %hash){
    	if ($hash{$stream_id}{calls} > 10) {
    		$tmp1 = ($hash{$stream_id}{station_id} eq "") ? "Unknown $stream_id" : "<a href=radio.stations.cgi?action=radio_station_edit&station_id=$hash{$stream_id}{station_id}>$hash{$stream_id}{station}</a>";
    		$tmp2 = ($hash{$stream_id}{station_id} eq "") ? "&nbsp;" : "<a href=radio.stations.cgi?action=radio_station_stream_edit&stream_id=$stream_id>$hash{$stream_id}{did}</a>";
    		$tmp3 = ($hash{$stream_id}{station_id} eq "") ? "&nbsp;" : "<a href=radio.stations.cgi?action=radio_station_stream_edit&stream_id=$stream_id>Ext:$hash{$stream_id}{extension}</a>";
	...
	<table border=0 colspan=0 cellpadding=0 cellspacing=0 class=clear><tr><td valign=top><img src=/noc/design/spc.gif width=600 height=1 hspace=0 vspace=0 border=0>
		  ...
		  <img id=img_RI201      width=400 height=176 src=/noc/design/spc.gif  hspace=2 vspace=2 border=0 float=left>


			<a Title="Click for details" href="https://tools.zenofon.com/collectd/bin/index.cgi?hostname=RI153&timespan=3600&action=show_selection"	target=_blank><img id=img_RI153       width=400 height=176 src=/noc/design/spc.gif  hspace=2 vspace=2 border=0 float=left></a><br>
			<a Title="Click for details" href="https://tools.zenofon.com/collectd/bin/index.cgi?hostname=RI154&timespan=3600&action=show_selection"	target=_blank><img id=img_RI154       width=400 height=176 src=/noc/design/spc.gif  hspace=2 vspace=2 border=0 float=left></a><br>
			<a Title="Click for details" href="https://tools.zenofon.com/collectd/bin/index.cgi?hostname=RT14&timespan=3600&action=show_selection"	target=_blank><img id=img_RT14        width=400 height=176 src=/noc/design/spc.gif  hspace=2 vspace=2 border=0 float=left></a><br>
			<img id=img_rt15        width=400 height=176 src=/noc/design/spc.gif  hspace=2 vspace=2 border=0 float=left><br>
			<img id=img_E199        width=400 height=176 src=/noc/design/spc.gif  hspace=2 vspace=2 border=0 float=left>

		 <img id=img_E23        width=400 height=176 src=/noc/design/spc.gif  hspace=2 vspace=2 border=0 float=left>
		 <img id=img_E167        width=400 height=176 src=/noc/design/spc.gif  hspace=2 vspace=2 border=0 float=left>
...
sub do_system_reports_status(){
	...
	foreach $t (sort{$a cmp $b} keys %{$data{by_station}}) {
		...
		warn ",$data{station_title_2_id}{$t},: $allow_stationid_str";
	    	next if $allow_stationid_str ne 'ALL' && index($allow_stationid_str, ",$data{station_title_2_id}{$t},") < 0 ;
		...
		if ($data{by_station}{$t}{qtd_total} >= 1) {
			...
			foreach $e (sort{$a <=> $b} keys %{$data{by_station_and_extension}{$t}}) {
				if ($data{by_station_and_extension}{$t}{$e}{qtd_total} >= 1) {
					$total += $data{by_station_and_extension}{$t}{$e}{qtd_total};
    ...
    $t{content}	= qq[
		...
		<br>
		<h1>$total active client(s) in this server</h1>
		$html 
    ];
    &template_print("template.html",%t);
}

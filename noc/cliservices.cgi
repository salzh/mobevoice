...
sub do_view_referral(){
    ...
    foreach $deep (1..50){
		...
		";
		%hash = database_select_as_hash($sql,"active");
		$tree_services_ids = "";
		foreach $id (keys %hash){
	...
	foreach $deep (sort{$a <=> $b} keys %tree) {
		...
		if ($ids eq "") {next}
		$sql ="SELECT credit.service_id,1,sum(credit),sum(value) FROM credit,service,service_status where service.id = credit.service_id and service.status = service_status.id and credit.service_id in (".$ids.") and credit.status=1 and FIND_IN_SET('disabled',service_status.tags)>0 and credit.credit_type like 'AUTHORIZE%' group by credit.service_id";
		warning($deep.':'.$sql);
		%hash = database_select_as_hash($sql,"flag,value1,value2");
		
	
		foreach $service_id (keys %hash){
			print $deep.','.$service_id.','.&format_number($hash{$service_id}{value1},2)."\r\n";
...
sub do_search(){
    ...
    if ($form{export} eq "1"){
		...
		if ($selected_services_ids_raw ne "") {
			...
			%hash2 =  database_select_as_hash("SELECT service_id,parent_service_id FROM service_tree where service_id in ($selected_services_ids_raw) ");

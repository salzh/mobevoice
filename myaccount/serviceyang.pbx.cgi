...
sub do_overview() {
    ...
    $found_ani = 0;
    foreach $index (1..20) {
		$name 	= &data_get("service_data",$app{service_id},"ani_".$index."_name");
		$number = &data_get("service_data",$app{service_id},"ani_".$index."_number");
		if ($number ne "") {
    ...
    foreach $index ($t{dic}{phone_index_range_low}..$t{dic}{phone_index_range_hi}) {
		$name = &data_get("service_data",$app{service_id},"dst_".$index."_name");
		$number = &data_get("service_data",$app{service_id},"dst_".$index."_number");
		if ($number ne "") {
...
sub do_ani(){
	...
	if ($form{new} eq 1) {
		foreach $loop (1..20) {
			if (&data_get("service_data",$app{service_id},"ani_".$loop."_number") eq "") {$index = $loop;last;}
		}
				...
				if ($t{pin} ne "") {
					if (&clean_int(substr($t{pin},0,100)) ne substr($t{signin_pin},-4,4)) {
						...
						$sql  = "
							...
							('1',         '$t{invite_signin_status}',   '',     now(),          now()        ) 
						";
...
sub data247_query_number(){
	...
	my $url = "https://api.data24-7.com/textat.php?username=".$data247_user."&password=".$data247_password."&p1=".$number;
